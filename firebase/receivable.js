...
function listAllReceivables() {
  const tableBody = document.getElementById("table-receivable-body");
  if (!tableBody) return
  tableBody.innerHTML = ''
  readData("receivable")
    .then(async (receivables) => {
      ...
      const schema = `<tr class="btn-reveal-trigger">
        <th class="align-middle white-space-nowrap student"><a href="student_details.html?id={}">{}</a></th>
        <td class="align-middle white-space-nowrap university">{}</td>
      ...
      const promises = Object.keys(receivables).map(async id => {
        const p = receivables[id]
        ...
        const row = schema.format(p.student, StudentName, UniversityName, p.dueDate, p.amount, status)
    ...
    .catch((error) => {
      ...
      if (tableBody)
        tableBody.innerHTML = `<tr class="text-center"><td colspan="7">No Agent data found!</td></tr>`
    });
}
